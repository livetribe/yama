...
// Package generator runs one generation over a set of target packages. A
// caller starts a run with a Driver.
//
// A run reads each target package. It writes the two files that Google Wire's
// load needs. It invokes Google Wire one time. It then writes a lifecycle file
// for each package that declares a lifecycle stub. A package that declares no
// such stub takes no part in a run.
//
// A run leaves each directory as it found it. It takes out every file that it
// wrote for Google Wire. It puts back every file that it moved.
package generator
