...
// file that carries it, and refuses a file that does not.
...
// stubs, in the order the package declares them.
...
// it wrote over. write replaces a file that carries generatedMarker, which a
// run that ended early leaves behind. It reports every other file, and the file
// keeps the bytes it holds.
...
// Wire takes them as one value, and every load that Yama makes takes them one
// at a time.
//
