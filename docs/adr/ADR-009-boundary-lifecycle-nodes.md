# ADR-009: Boundary Lifecycle Nodes
...
must run before every graph node, or after every graph node, and it has no genuine
dependency relationship to any particular graph node.

Expressing "run before all the others" or "run after all the others" through the
construction graph is possible but brittle. It requires wiring a node so that every
...
Yama needs a way to register such nodes at the extremes of the lifecycle without
...
Boundary nodes are peers of the graph nodes. A boundary node participates in the
lifecycle through whichever of `Starter`, `Quiescer`, and `Stopper` it implements,
exactly as a graph node does. The boundary a node is registered in does not change
what it does; it controls only its execution order relative to the graph:

* A **begin** node runs before all graph nodes in each pass it participates in.
* An **end** node runs after all graph nodes in each pass it participates in.

The following properties hold:

* Each boundary is a flat, unordered set. There is no ordering guarantee among nodes
  in the same set.
* A boundary node has no dependency relationship to any graph node. A node that has a
  real dependency on, or dependents in, the graph belongs in the graph, not in a
  boundary.
* Execution is best-effort. A failing or panicking boundary node does not prevent
  the pass from proceeding.
* Boundary nodes run under the same caller context as the pass they join and share
  its deadline; Yama gives them no budget of their own. A slow begin node consumes
...
The mechanism exists to express one thing: this node runs before everything, or
...
Because boundary nodes are peers, a boundary registration changes only gross
ordering — front or back — and nothing else. A node's behavior comes entirely from
the lifecycle interfaces it implements, the same interfaces a graph node uses.
...
specific other nodes has a real dependency relationship and belongs in the
...
Best-effort execution follows the shutdown philosophy of ADR-006: a boundary node
cannot fail a pass any more than a graph node can.
...
* Boundary nodes reuse the existing lifecycle interfaces and the existing passes;
...
* Boundary nodes consume the shared deadline of the pass they run in, so a slow
  begin node leaves less time for the rest of that pass. This is documented, not
  mitigated.
* Because execution is best-effort, a boundary node's failure is invisible to the
...
Rejected because ordering relative to specific nodes is a real dependency
...
### Boundary Nodes With Graph Dependencies

Rejected because a node with a genuine dependency on, or dependents in, the graph is
a graph node. A boundary node exists precisely for work that has no such
...
Rejected because boundary nodes do not add a phase. They reuse the existing Start,
Quiesce, and Stop passes and preserve the fixed three-phase model of ADR-003.

## Non-Goals

Boundary nodes do not provide:

* Ordering among nodes in the same boundary set.
* A dependency relationship between boundary nodes and graph nodes.
* A new lifecycle phase.
* A guarantee that a boundary node completes or succeeds.

Boundary nodes exist only to register work that runs before all graph nodes or after
all graph nodes, without expressing that position through the construction graph.
